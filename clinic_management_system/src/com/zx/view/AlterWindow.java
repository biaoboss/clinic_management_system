 ...
 *
 */
public class AlterWindow extends JDialog {
    ...
    public AlterWindow(AddSalWindow addSal, ProListVo listVo) {
        ...
        pro_amount.setText(String.valueOf(listVo.getAcount()));
...
//		确认按钮动作监听器
        btn1.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                Object obj = addSal.getListdao().getStockById(id);
                if(obj != null) {
                    int stock = Integer.parseInt(obj.toString());
                    int amountInt =listVo.getAcount();
                    if(buyAmount>amountInt+stock) {
                        JOptionPane.showMessageDialog(AlterWindow.this, "采购数量超过库存数！");
                        return ;
                    }else {
                        addSal.getListdao().updateStockById(id, amountInt+stock- buyAmount);
                    }
                    DefaultTableModel model = addSal.getListmodel();
                    Vector datas = addSal.getListdao().getProList();
                    model.setRowCount(0);
                    for (int i = 0; i < datas.size(); i++) {
                        model.addRow((Vector)datas.get(i));
                    }
                }
                ...
                int row = addSal.protable.getSelectedRow();
                model.addRow(data);
                model.removeRow(row);
                AlterWindow.this.dispose();
            }
        });
//		退出按钮动作监听器
        btn2.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                AlterWindow.this.setVisible(false);
