...
import com.zx.control.PatientModifyAction;
 ...
 *
 */
public class PatientModifyDialog extends JDialog {
	...
	private PatientManagementDialog manageDialog;
	PatientVo patVo;
	ShowBasicInfomationPane showPane;
	
	public PatientModifyDialog(PatientVo patVo,PatientManagementDialog manageDialog,ShowBasicInfomationPane showPane) {
		this.patVo=patVo;
		this.manageDialog=manageDialog;
		this.showPane=showPane;
		//按钮实例化
		btnKeep=new JButton("保存");
		btnKeep.addActionListener(new PatientModifyAction(this,manageDialog,showPane));
		btnQuit=new JButton("退出");
		btnQuit.addActionListener(new PatientModifyAction(this,manageDialog,showPane));
		...
		//日期下拉列表框实例化
		String[] birth = {};
		boxBrith=new JComboBox(birth) ;
		
		//设置文本框内的值
	    txtId.setText(patVo.getId());
		txtName.setText(patVo.getName());
		boxSex.setSelectedItem(patVo.getSex());
		txtAge.setText(patVo.getAge()+"");
		txtAddress.setText(patVo.getAddress());
		txtPhone.setText(patVo.getPhone());
		txtNation.setText(patVo.getNation());
		txtIdCard.setText(patVo.getIdCard());
		txtJob.setText(patVo.getJob());
		txtHeight.setText(patVo.getHeight()+"");
		txtWeight.setText(patVo.getWeight()+"");
		txtBloodPressure.setText(patVo.getTemperature()+"");
		txtTemperature.setText(patVo.getTemperature()+"");
		txtRemarks.setText(patVo.getRemarks());
		boxBrith.addItem(patVo.getBrith());
		...
		this.setLocationRelativeTo(manageDialog);
